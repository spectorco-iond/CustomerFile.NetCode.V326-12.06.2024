...
#Region "Public constructors ##############################################"
    ...
    Public Sub New(ByVal pID As String)
        ...
        Catch er As Exception
            MsgBox("Error in cCicntp100ESE." & New Diagnostics.StackFrame(0).GetMethod.Name & vbCrLf & er.Message)
        End Try

    End Sub

    Public Sub New(ByRef p_Cntp As cCicntp, ByVal cnt_id As String, ByVal cmp_wwn As String)

        Try

            Call Init()

            Call LoadObject(p_Cntp, cnt_id, cmp_wwn)
...
#End Region
    ...
    Public Sub LoadObject(ByRef p_Cntp As cCicntp, ByVal cnt_id As String, ByVal cmp_wwn As String)
        Try
            ...
            With p_Cntp
                ...
                m_sysmodified = .sysmodified
                m_numberfield1 = .Alternate_Rep
                m_YesNoField3 = .YesNoField3
                m_textfield1 = .Use_Account_No
        ...
        Catch ex As Exception
            MsgBox("Error in cCicntp100ESE." & New Diagnostics.StackFrame(0).GetMethod.Name & vbCrLf & ex.Message)
        End Try
    End Sub
...
#Region "Private maintenance procedures ###################################"
    ...
    Private Sub Load(ByVal pintID As Integer)
        ...
        Try
            ...
            strSql =
            "SELECT * " &
            "FROM   [100ESE].dbo.cicntp  WITH (Nolock) " &
            "WHERE  cnt_id = " & pintID & " "

            '  "WHERE ISNULL(active_y,0) = 1 And cnt_id = " & pintID & " "
    ...
    Private Sub LoadLine(ByRef pdrRow As DataRow)
        ...
        Try
            ...
            'If Not (pdrRow.Item("cnt_id").Equals(DBNull.Value)) Then m_cnt_id = pdrRow.Item("cnt_id")
            'If Not (pdrRow.Item("cmp_wwn").Equals(DBNull.Value)) Then m_cmp_wwn = pdrRow.Item("cmp_wwn")
            ...
            'If Not (pdrRow.Item("sysguid").Equals(DBNull.Value)) Then m_sysguid = pdrRow.Item("sysguid")
            'If Not (pdrRow.Item("timestamp").Equals(DBNull.Value)) Then m_timestamp = pdrRow.Item("timestamp")
            ...
            ' m_numberfield1 will contain Alternate Rep humres.Res_ID number
            ...
            ' m_textField1 will contain Alternate customer account number
            If Not (pdrRow.Item("textfield1").Equals(DBNull.Value)) Then m_textfield1 = pdrRow.Item("textfield1").ToString
    ...
    Private Sub SaveLine(ByRef pdrRow As DataRow)
        ...
        Try
            ...
            'SqlCompliantString(m_cnt_acc_man) & "', " & " 
            ...
            If m_ID = 0 Then ' We only want to update these fields on first save
                pdrRow.Item("cmp_wwn") = m_cmp_wwn
                pdrRow.Item("Administration") = "100"
                pdrRow.Item("syscreator") = 1
                pdrRow.Item("sysmodifier") = 1
                pdrRow.Item("numberfield2") = 0
                pdrRow.Item("numberfield3") = 0
                pdrRow.Item("numberfield4") = 0
                pdrRow.Item("numberfield5") = 0
            End If
...
#Region "Public maintenance procedures ####################################"
    ...
    Public Sub Save()
        ...
        Try
            ...
            strSql = " SELECT * FROM [100ESE].dbo.cicntp  WHERE cnt_id = '" & m_cnt_id & "' "
            ...
            If dt.Rows.Count = 0 Then
                ...
                Dim fullname As String = m_FullName


                strSql = "INSERT INTO [100ESE].dbo.cicntp (cnt_id,cnt_f_name,cnt_l_name,FullName,Initials,predcode,Gender,taalcode,cnt_f_ext,cnt_f_fax,cnt_f_tel,cnt_f_mobile,cnt_email,cmp_wwn,Administration,
                               syscreator,sysmodifier,numberfield2,numberfield3,numberfield4,numberfield5,numberfield1,cnt_acc_man, active_y, sysmodified, YesNoField3, textfield1)  values ('" & m_cnt_id & "','" & Trim(m_cnt_f_name) & "','" & Trim(m_cnt_l_name) & "',
'" & Trim(m_FullName) & "','" & m_Initials & "','" & m_predcode & "','" & Trim(m_Gender) & "','" & m_taalcode & "','" & Trim(m_cnt_f_ext) & "','" & Trim(m_cnt_f_fax) & "','" & Trim(m_cnt_f_tel) & "','" & Trim(m_cnt_f_mobile) & "',
                               '" & Trim(m_cnt_email) & "','" & m_cmp_wwn & "','100',1,1,0,0,0,0," & m_numberfield1 & ",'" & m_cnt_acc_man & "'," & m_active_y & ",'" & sysmodified & "'," & m_YesNoField3 & ",'" & m_textfield1 & "')"
            ...
            Else
                ...
                strSql = "Update [100ESE].dbo.cicntp set cnt_id = '" & m_cnt_id & "',cnt_f_name ='" & Trim(m_cnt_f_name) & "', cnt_l_name= '" & Trim(m_cnt_l_name) & "', FullName ='" & Trim(m_cnt_f_name) & " " & Trim(m_cnt_l_name) & "',
Initials = '" & m_Initials & "', predcode = '" & m_predcode & "',Gender = '" & Trim(m_Gender) & "', taalcode ='" & Trim(m_taalcode) & "', cnt_f_ext ='" & Trim(m_cnt_f_ext) & "',cnt_f_fax = '" & Trim(m_cnt_f_fax) & "', cnt_f_tel ='" & Trim(m_cnt_f_tel) & "',
cnt_f_mobile= '" & Trim(m_cnt_f_mobile) & "',cnt_email ='" & Trim(m_cnt_email) & "',numberfield1 = " & m_numberfield1 & ",cnt_acc_man= '" & m_cnt_acc_man & "',active_y= " & m_active_y & ",sysmodified='" & m_sysmodified & "',YesNoField3 = " & m_YesNoField3 & ",textfield1 = '" & m_textfield1 & "'
where cnt_id = '" & m_cnt_id & "' "
            ...
            If m_ID = 0 Then ' We only want to do this one time at entry, not at mod...

                m_ID = db.DataTable("Select MAX(Id) As Max_Id FROM [100ESE].[dbo].cicntp  With (Nolock)").Rows(0).Item("Max_Id")

                ' Retrieve Contact info
                strSql = " Select TOP 1 * FROM [100ESE].[dbo].cicntp  With (Nolock) WHERE  cmp_wwn = '" & m_cmp_wwn & "' AND ID = " & m_ID.ToString
                dt = db.DataTable(strSql)

                ' Add Contact info to Addresses, ShippingDays and BacoDiscussionStdPictures tables
                strSql = " INSERT INTO [100ESE].[dbo].Addresses ( Type, Account, ContactPerson, AddressLine1, AddressLine2, AddressLine3, " & " PostCode, City, StateCode, County, Country, KeepSameAsVisit, textfield1, Main ) " & " VALUES ( 'DEL', '" & m_cmp_wwn & "', '" & dt.Rows(0).Item("cnt_id").ToString & "', '" & m_Customer.cmp_fadd1.Replace("'", "''") & "', " & " '" & m_Customer.cmp_fadd2.Replace("'", "''") & "', '" & m_Customer.cmp_fadd3.Replace("'", "''") & "', '" & m_Customer.cmp_fpc.Replace("'", "''") & "', " & " '" & m_Customer.cmp_fcity.Replace("'", "''") & "', '" & m_Customer.cmp_fcounty.Replace("'", "''") & "', '" & m_Customer.StateCode & "', " & " '" & m_Customer.cmp_fctry & "', '1', '" & m_Customer.cmp_name.Replace("'", "''") & "', '0' ) "
                db.Execute(strSql)

                '++ID 10.17.2016 put in comment because we don't need insert POS int the Type
                'strSql = " INSERT INTO Addresses ( Type, Account, ContactPerson, AddressLine1, AddressLine2, AddressLine3, " & " PostCode, City, StateCode, County, Country, KeepSameAsVisit, textfield1, Main ) " & " VALUES ( 'POS', '" & m_cmp_wwn & "', '" & dt.Rows(0).Item("cnt_id").ToString & "', '" & m_Customer.cmp_fadd1.Replace("'", "''") & "', " & " '" & m_Customer.cmp_fadd2.Replace("'", "''") & "', '" & m_Customer.cmp_fadd3.Replace("'", "''") & "', '" & m_Customer.cmp_fpc.Replace("'", "''") & "', " & " '" & m_Customer.cmp_fcity.Replace("'", "''") & "', '" & m_Customer.cmp_fcounty.Replace("'", "''") & "', '" & m_Customer.StateCode & "', " & " '" & m_Customer.cmp_fctry & "', '1', '" & m_Customer.cmp_name.Replace("'", "''") & "', '0' ) "
                'db.Execute(strSql)

                strSql =
                    " SELECT ID " &
                    " FROM [100ESE].[dbo].Addresses " &
                    " WHERE Account = '" & m_cmp_wwn & "'" & " AND ContactPerson = '" & dt.Rows(0).Item("cnt_id").ToString & "' " &
                    " ORDER BY ID DESC "
                ...
                strSql = " INSERT INTO [100ESE].[dbo].ShippingDays ( AddressID ) " & " VALUES ( '" & dtAddressInfo.Rows(0).Item("ID").ToString & "' ) "
                db.Execute(strSql)

                strSql = " INSERT INTO [100ESE].[dbo].BacoDiscussionStdPictures ( Description, Filename, Type, Width, Height, Size, Division ) " & " VALUES ( '_" & m_Customer.cmp_code.Replace("'", "''") & "', '" & m_FullName.Replace("'", "''") & "', 'vCard', " & " '1', '1', '1', '100' ) "
                db.Execute(strSql)

                '++ID in I put in comments because in 100ESE don't exist this table this means Synergy don't need 
                'strSql = " INSERT INTO EXACT_TRAVELER_NEW_CONTACTS_FROM_APP ( cnt_id, cmp_wwn, AddingUser ) " & " VALUES ( '" & dt.Rows(0).Item("cnt_id").ToString & "', '" & m_Customer.cmp_wwn & "', '" & Environment.UserName & "' ) "
                'db.Execute(strSql)
...
#Region "Public properties ################################################"
    ...
    Public Property Alternate_Rep() As Integer
        ...
        End Set
    End Property
    Public Property YesNoField3 As Integer
        Get
            YesNoField3 = m_YesNoField3
        End Get
        Set(ByVal value As Integer)
            m_YesNoField3 = value
        End Set
