Public Class cCicntp
    ...
    Dim m_Contact100ESE As cCicntp100ESE
    Dim m_Contact As cCicntp
...
#Region "Private maintenance procedures ###################################"
    ...
    Private Sub Load(ByVal pintID As Integer)
        ...
        Try
            ...
            '++ID 11.13.2019 added criteria for exclude inactive contacts C.active_y = 1 
    ...
    Private Sub SaveLine(ByRef pdrRow As DataRow)
        ...
        Try
            ...
            m_FullName = m_cnt_f_name & " " & m_cnt_l_name ' m_FullName
...
#Region "Public maintenance procedures ####################################"
    ...
    Public Sub Save()
        ...
        Try
            ...
            '  m_Contact100ESE = New cCicntp100ESE
            ...
            ' "WHERE ISNULL(active_y,0) = 1 and ID = " & m_ID & " "
            ...
            If m_ID = 0 Then ' We only want to do this one time at entry, not at mod...
                ...
                m_ID = db.DataTable("SELECT MAX(Id) AS Max_Id FROM Cicntp WITH (Nolock)").Rows(0).Item("Max_Id")


                ' Retrieve Contact info
                strSql = " SELECT TOP 1 * FROM cicntp WITH (Nolock) WHERE cmp_wwn = '" & m_cmp_wwn & "' AND ID = " & m_ID.ToString


                'strSql = " SELECT TOP 1 * FROM cicntp WITH (Nolock) WHERE ISNULL(active_y,0) = 1 and cmp_wwn = '" & m_cmp_wwn & "' AND ID = " & m_ID.ToString
                ...
                '----------------------------------------------------------
                '++ID 1.24.25.2017
                'before serialise the class
                m_Contact = New cCicntp(m_ID)

                m_Contact100ESE = New cCicntp100ESE(m_Contact, dt.Rows(0).Item("cnt_id").ToString, dt.Rows(0).Item("cmp_wwn").ToString)
                m_Contact100ESE.cnt_id = dt.Rows(0).Item("cnt_id").ToString
                m_Contact100ESE.cmp_wwn = dt.Rows(0).Item("cmp_wwn").ToString
                m_Contact100ESE.Customer = m_Customer
                m_Contact100ESE.Save()
                '-----------------------------------------------------------
                ...
                strSql = " INSERT INTO BacoDiscussionStdPictures ( Description, Filename, Type, Width, Height, Size, Division ) " & " VALUES ( '_" & m_Customer.cmp_code.Replace("'", "''") & "', '" & m_FullName.Replace("'", "''") & "', 'vCard', " & " '1', '1', '1', '100' ) "
                db.Execute(strSql)
                ...
                '++ID 1.24.25.2017 added else with all
                '----------------------------------------------------------
            Else
                ' Retrieve Contact info
                strSql = " SELECT TOP 1 * FROM cicntp WITH (Nolock) WHERE  cmp_wwn = '" & m_cmp_wwn & "' AND ID = " & m_ID.ToString
